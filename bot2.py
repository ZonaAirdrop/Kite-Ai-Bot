...
class KiteAi:
    def __init__(self) -> None:
        self.KITE_AI = {
            ...
            "tokens": [
                ...
                { "type": "erc20", "ticker": "USDT", "address": "0x0fF5393387ad2f9f691FD6Fd28e07E3969e27e63" }  # Ditambahkan USDT
            ],
            "chain_id": 2368
        }
        self.BASE_SEPOLIA = {
        ...
        # Ditambahkan swap router dan ABI
    ...
    # Ditambahkan fungsi untuk menampilkan pertanyaan swap
    def print_swap_question(self):
        while True:
            try:
                swap_amount = float(input(f"{Colors.YELLOW + Colors.BOLD}Enter Swap Amount? -> {Colors.RESET}").strip())
                if swap_amount > 0:
    ...
    # Ditambahkan fungsi untuk melakukan swap
    async def perform_swap(self, account: str, address: str, rpc_url: str, token_in: str, token_out: str, amount: float, use_proxy: bool):
        try:
            web3 = await self.get_web3_with_check(address, rpc_url, use_proxy)
            
            # Temukan token berdasarkan ticker
            token_in_obj = next(t for t in self.KITE_AI["tokens"] if t["ticker"] == token_in)
            token_out_obj = next(t for t in self.KITE_AI["tokens"] if t["ticker"] == token_out)
            
            token_contract = web3.eth.contract(
                address=web3.to_checksum_address(token_in_obj["address"]),
                abi=self.ERC20_CONTRACT_ABI
            )
            ...
            # Periksa allowance dan approve jika diperlukan
            allowance = token_contract.functions.allowance(address, self.SWAP_ROUTER_ADDRESS).call()
            if allowance < amount_to_wei:
                approve_data = token_contract.functions.approve(self.SWAP_ROUTER_ADDRESS, amount_to_wei)
                ...
                await self.wait_for_receipt_with_retries(web3, tx_hash)
                logger.info(f"Approval Tx: {self.KITE_AI['explorer']}{tx_hash}")
                await asyncio.sleep(10)  # Tunggu konfirmasi
            
            # Bangun transaksi swap
            swap_data = swap_contract.functions.initiate(
                web3.to_checksum_address(token_in_obj["address"]),
                amount_to_wei,
                (
                    1,  # sourceId
                    address,  # receiver
                    ...
                    [(
                        1,  # action (trade)
                        200000,  # requiredGasLimit
                        100000,  # recipientGasLimit
                        b'\x00',  # trade data placeholder
                        (
                            web3.to_checksum_address("0xD1bd49F60A6257dC96B3A040e6a1E17296A51375"),  # bridgeSourceChain
                            False,  # sourceBridgeIsNative
                            web3.to_checksum_address("0x0000000000000000000000000000000000000000"),  # bridgeDestinationChain
                            web3.to_checksum_address("0x0000000000000000000000000000000000000000"),  # cellDestinationChain
                            b'\x00' * 32,  # destinationBlockchainID
                            0,  # teleporterFee
    ...
    # Diperbarui untuk menambahkan menu swap
    def print_question(self):
        while True:
            try:
                ...
                print(f"{Colors.WHITE + Colors.BOLD}7. Swap Token (KITE<->USDT){Colors.RESET}")  # Ditambahkan menu swap
                print(f"{Colors.WHITE + Colors.BOLD}8. Run All Features{Colors.RESET}")  # Diubah menjadi 8
                ...
                if option in [1, 2, 3, 4, 5, 6, 7, 8]:
                    option_type = (
                        ...
                        "Swap Token" if option == 7 else  # Ditambahkan
                        "Run All Features"
                    )
                    print(f"{Colors.GREEN + Colors.BOLD}{option_type} Selected.{Colors.RESET}")
                    break
                else:
                    print(f"{Colors.RED + Colors.BOLD}Please enter either 1-8.{Colors.RESET}")
            except ValueError:
                print(f"{Colors.RED + Colors.BOLD}Invalid input. Enter a number (1-8).{Colors.RESET}")
        ...
        if option == 5:
        ...
        elif option == 7:  # Ditambahkan untuk swap
            self.print_swap_question()

        elif option == 8:  # Diubah menjadi 8
            while True:
            ...
            while True:  # Ditambahkan untuk auto swap
                auto_swap = input(f"{Colors.YELLOW + Colors.BOLD}Auto Perform Swap? [y/n] -> {Colors.RESET}").strip()
                if auto_swap in ["y", "n"]:
    ...
    # Ditambahkan fungsi untuk memproses swap
    async def process_option_swap(self, account: str, address: str, use_proxy: bool):
        logger.step("Swap Token:")
        
        # Tentukan pasangan token (KITE ke USDT atau USDT ke KITE)
        swap_pairs = [
            ("KITE", "USDT"),
            ("USDT", "KITE")
        ]
        ...
        token_in_obj = next(t for t in self.KITE_AI["tokens"] if t["ticker"] == token_in)
        balance = await self.get_token_balance(address, self.KITE_AI["rpc_url"], token_in_obj["address"], token_in_obj["type"], use_proxy)
        
        logger.info(f"    Swap    : {token_in} ➡ {token_out}")
        logger.info(f"    Balance : {balance} {token_in}")
        logger.info(f"    Amount  : {self.swap_amount} {token_in}")
        
        if not balance or balance < self.swap_amount:
            logger.warn(f"Insufficient {token_in} balance")
            return
    ...
    # Ditambahkan pemanggilan untuk swap
    async def process_option_7(self, account: str, address: str, use_proxy: bool):
        await self.process_option_swap(account, address, use_proxy)
    ...
    async def process_accounts(self, account: str, address: str, option: int, use_proxy: bool, rotate_proxy: bool):
        ...
        if signed:
            ...
            if option == 1:
                ...
                if self.auto_swap:  # Ditambahkan untuk auto swap
                    await self.process_option_7(account, address, use_proxy)
