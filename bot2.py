...
class KiteAi:
    ...
    def generate_swap_option(self):
        ...
        token_in_ticker, token_out_ticker = random.choice(token_pairs)
        
        token_in = next(token for token in chain["tokens"] if token["ticker"] == token_in_ticker)
        token_out = next(token for token in chain["tokens"] if token["ticker"] == token_out_ticker)
    ...
    async def perform_swap(self, account: str, address: str, rpc_url: str, token_in: dict, token_out: dict, amount: float, explorer: str, use_proxy: bool):
        try:
            web3 = await self.get_web3_with_check(address, rpc_url, use_proxy)
            
            token_in_address = web3.to_checksum_address(token_in["address"])
            token_in_type = token_in["type"]
            ...
            if token_in_type == "erc20":
                await self.approving_token(
                    account, address, rpc_url, 
                    self.SWAP_ROUTER_ADDRESS, token_in_address, 
                    amount_to_wei, explorer, use_proxy
                )
            ...
            # Build instructions - simplified for demo
            instructions = (
                1,  # sourceId
                address,  # receiver
                ...
                [(
                    1,  # action (1 = swap)
                    100000,  # requiredGasLimit
                    100000,  # recipientGasLimit
                    b'',  # trade data (empty for demo)
                    (
                        b'\x00' * 20,  # bridgeSourceChain
                        False,  # sourceBridgeIsNative
                        b'\x00' * 20,  # bridgeDestinationChain
                        b'\x00' * 20,  # cellDestinationChain
    ...
    def print_question(self):
        while True:
            try:
                ...
                print(f"{Colors.WHITE + Colors.BOLD}7. Swap Token{Colors.RESET}")
        ...
        if option == 5:
            ...
            while True:
                auto_swap = input(f"{Colors.YELLOW + Colors.BOLD}Auto Perform Token Swap? [y/n] -> {Colors.RESET}").strip()
                if auto_swap in ["y", "n"]:
    ...
    async def process_perform_swap(self, account: str, address: str, rpc_url: str, token_in: dict, token_out: dict, amount: float, explorer: str, use_proxy: bool):
        tx_hash, block_number = await self.perform_swap(account, address, rpc_url, token_in, token_out, amount, explorer, use_proxy)
    ...
    async def process_option_7(self, account: str, address: str, use_proxy: bool):
        logger.step("Swap Transaction:")

        for i in range(self.swap_count):
            logger.info(f"Swap {i+1} / {self.swap_count}")

            swap_data = self.generate_swap_option()
            rpc_url = swap_data["rpc_url"]
            explorer = swap_data["explorer"]
            token_in = swap_data["token_in"]
            token_out = swap_data["token_out"]
            
            token_in_ticker = token_in["ticker"]
            token_out_ticker = token_out["ticker"]
            
            amount = round(random.uniform(self.min_swap_amount, self.max_swap_amount), 6)

            balance = await self.get_token_balance(address, rpc_url, token_in["address"], token_in["type"], use_proxy)

            logger.info(f"    Token In : {token_in_ticker}")
            logger.info(f"    Token Out: {token_out_ticker}")
            logger.info(f"    Balance  : {balance} {token_in_ticker}")
            logger.info(f"    Amount   : {amount} {token_in_ticker}")

            if not balance or balance < amount:
                logger.warn(f"Insufficient {token_in_ticker} Token Balance. Skipping swap.")
                continue

            await self.process_perform_swap(account, address, rpc_url, token_in, token_out, amount, explorer, use_proxy)
            await self.print_timer("Tx")
